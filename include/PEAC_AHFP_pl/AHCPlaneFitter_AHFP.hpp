...
#include "AHCTypes_AHFP.hpp"
#include "AHCPlaneSeg_AHFP.hpp"
#include "AHCParamSet_AHFP.hpp"
#include "AHCUtils_AHFP.hpp"
...
#undef TMP_LOG_VAR
	...
	protected:
			...
			cv::Mat* pSeg)
		{
			...
			planes.resize(extractedPlanes.size());
			// cv::Mat need_image(this->height, this->width, CV_8UC1, cv::Scalar(0));
			// needed_image = cv::Mat(this->height, this->width, CV_8UC1, cv::Scalar(0));
			for (size_t i = 0; i < extractedPlanes.size(); i++)
			{
				planes.at(i) = cv::Mat::zeros(this->height, this->width, CV_8UC1);
			...
			/**
			 * @brief 添加只考虑两平面存在夹角且相交的项
			 * 
			 */
			// 先判断那两个平面存在夹角
			vector<std::pair<int, int>> angle_pair;
			for (int i = 0; i < extractedPlanes.size(); i++)
			{
				for (int j = i + 1; j < extractedPlanes.size(); j++)
				{
					Eigen::Vector3d normal_i(extractedPlanes.at(i)->normal[0], extractedPlanes.at(i)->normal[1], extractedPlanes.at(i)->normal[2]);
					Eigen::Vector3d normal_j(extractedPlanes.at(j)->normal[0], extractedPlanes.at(j)->normal[1], extractedPlanes.at(j)->normal[2]);
					if (abs(normal_i.dot(normal_j)) < 0.985) //判断两个平面角度阈值
					{
						angle_pair.emplace_back(std::make_pair(i, j));
					}
				}
			}
			
			unfeasible_image = cv::Mat::zeros(this->height, this->width, CV_8UC1);
			for (auto & pair : angle_pair)
			{
				// cv::Mat Gaussian_filter_image1;
      			// cv::GaussianBlur(planes.at(pair.first), Gaussian_filter_image1, cv::Size(5, 5), 1, 1);

				// cv::Mat Gaussian_filter_image2;
      			// cv::GaussianBlur(planes.at(pair.second), Gaussian_filter_image2, cv::Size(5, 5), 1, 1);

				// 执行轮廓检测
				std::vector<std::vector<cv::Point>> contours_color1, contours_color2;
				cv::findContours(planes.at(pair.first), contours_color1, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
				cv::findContours(planes.at(pair.second), contours_color2, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);

				// 删除图像中的杂点
				auto itr = contours_color1.begin();    //使用迭代器去除噪声轮廓
				while (itr != contours_color1.end())
				{
					auto area = cv::contourArea(*itr);  //获得轮廓面积
					if (area < 8)    //删除较小面积的轮廓
					{
						itr = contours_color1.erase(itr); //itr一旦erase，需要重新赋值
					}
					else
					{
						itr++;
				...
				itr = contours_color2.begin();    //使用迭代器去除噪声轮廓
				while (itr != contours_color2.end())
				{
					auto area = cv::contourArea(*itr);  //获得轮廓面积
					if (area < 8)    //删除较小面积的轮廓
					{
						itr = contours_color2.erase(itr); //itr一旦erase，需要重新赋值
					}
					else
					{
						itr++;
				...
				// 对其中一个颜色的轮廓进行膨胀
				int dilation_size = 4;
				cv::Mat dilate_image1;
				cv::Mat element = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(2 * dilation_size + 1, 2 * dilation_size + 1));
				cv::dilate(planes.at(pair.first), dilate_image1, element);

				// 检查膨胀结果与另一个颜色的掩码是否有交集
				cv::Mat intersection2;
				cv::bitwise_and(dilate_image1, planes.at(pair.second), intersection2);

				// 判断是否存在交集
    			// bool are_adjacent = cv::countNonZero(intersection) > 0;

				// 图像2的轮廓
				// std::vector<cv::Point> intersecting_pixels2;
    			// cv::findNonZero(intersection2, intersecting_pixels2);

				cv::Mat dilate_image2;
				cv::dilate(planes.at(pair.second), dilate_image2, element);
				cv::Mat intersection1;
				cv::bitwise_and(dilate_image2, planes.at(pair.first), intersection1);
				// std::vector<cv::Point> intersecting_pixels1;
    			// cv::findNonZero(intersection1, intersecting_pixels1);
				cv::Mat unfeasible_image_tmp;
				// 如果存在边界
				if (cv::countNonZero(intersection2) > 0 && cv::countNonZero(intersection1) > 0)
				{
					// 将边界腐蚀
					cv::Mat dilate_intersection1, dilate_intersection2;
					cv::Mat element_ee = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(15, 15));
					cv::dilate(intersection2, dilate_intersection2, element_ee);
					cv::dilate(intersection1, dilate_intersection1, element_ee);
					cv::bitwise_or(dilate_intersection1, dilate_intersection2, unfeasible_image_tmp);

					cv::bitwise_or(unfeasible_image_tmp, unfeasible_image, unfeasible_image);
				}
			}
			...
			for (size_t i = 0; i < planes.size(); i++)
			{
				...
				cv::Mat Gaussian_filter_image;
      			cv::GaussianBlur(planes.at(i), Gaussian_filter_image, cv::Size(21, 21), 1, 1);
				...
				// 外圈逆时针，内圈顺时针https://stackoverflow.com/questions/45323590/do-contours-returned-by-cvfindcontours-have-a-consistent-orientation
				...
				{
					cv::approxPolyDP(coutour, *approx_contour_iter, 8, true);
					approx_contour_iter++;
				}
				extractedPlanes.at(i)->contours_2d = approx_contours;
				...
				*/
				...
				/**
				 * 注意：虽然opencv findcontours找到的轮廓外层是逆时针，内层是顺时针，这是在像素坐标系下的。但转到相机坐标系下后，轮廓方向会与像素坐标系下的方向相反，也就是变成外圈顺时针，内圈逆时针。
				*/
				...
				{
					contour_points.clear();
					// LOG(INFO)<<"current contour size "<<contour.size();
					if (contour.size() < 3)
					{
						continue;
					}
					...
					{
						// LOG(INFO)<<"OUTER";
						for (auto iter = contour.begin(); iter != contour.end(); iter++)
						{
							float div_z = normal(0) * (iter->x - kCx) / kFx + normal(1) * (iter->y - kCy) / kFy + normal(2);
							float z = (normal.dot(center)) / div_z;      
							float x = ((float)(iter->x) - kCx) * z / kFx;
							float y = ((float)(iter->y) - kCy) * z / kFy;
							Eigen::Vector3f tmppoint(x, y, z);
							// LOG(INFO)<<"X "<<iter->x<<" Y "<<iter->y;
							// LOG(INFO)<<tmppoint.transpose();
							contour_points.emplace_back(tmppoint);
						}
						// is_outer = false;
					}
					else// 这个地方是为了适用boost geometry的构造方式
					{
						// LOG(INFO)<<"INNER";
						for (auto riter = contour.rbegin(); riter != contour.rend(); riter++)
						{
