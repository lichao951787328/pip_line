...
#include "AHCTypes_AHFP.hpp"		//shared_ptr
#include "eig33sym_AHFP.hpp"		//PlaneSeg::Stats::compute
#include "AHCParamSet_AHFP.hpp"		//depthDisContinuous
#include "DisjointSet_AHFP.hpp"	//PlaneSeg::mergeNbsFrom
#include <vector>
#include <Eigen/Core>
...
struct PlaneSeg {
	...
	*/
	...
	std::vector<std::vector<Eigen::Vector3f>> contours; // this member is the contours of current plane, the outer is clockwise, the inners is counter clockwise
	std::vector<std::vector<cv::Point>> contours_2d; // this member is the contours of current plane in Mat, pixel
#ifdef DEBUG_INIT
	enum Type {
