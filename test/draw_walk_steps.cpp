...
// const int kDepthHeight = 720;
  ...
  return std::make_pair(i - 50, j -100);
}
...
void draw_steps(vector<footstep>& steps)
{
  ...
  for (auto & iter_step : steps)
  {
    ...
    if (iter_step.is_left)
    {
      ...
      cv::polylines(draw_image, fillContAll, true, cv::Scalar(255, 0, 0), 5); 
    }
    else
    {
      // cv::fillPoly(draw_image, fillContAll, cv::Scalar(0, 0, 255));
      cv::polylines(draw_image, fillContAll, true, cv::Scalar(0, 0, 255), 5); 
    }
...
{
  draw_image = cv::imread("/home/humanoid/work/plane_detection/catkin_plane_detection_test_ws/src/plane_detection/bag/walk_step/segmented_contours copy 8.png");
  ...
  // footstep tmpstep1(false, 0.21, -0.18, 0, 0);
  // footstep tmpstep2(true, 0.21, 0.12, 0, 0);

  // footstep tmpstep3(false, 0.49, -0.18, 0.1, 0);
  // footstep tmpstep4(true, 0.75, 0.12, 0.2, 0);

  // footstep tmpstep5(false, 1.01, -0.18, 0.3, 0);
  // footstep tmpstep6(true, 1.01, 0.12, 0.3, 0);
  ...
  cv::waitKey(0);
  cv::imwrite("/home/humanoid/work/plane_detection/catkin_plane_detection_test_ws/src/plane_detection/bag/walk_step/planed_steps_with_contours.png", draw_image);
  return 0;
}