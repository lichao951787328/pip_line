...
orginazed_points_pcl raw_points;
parameter param;
...
void initialRawPoints(size_t start_col_, size_t start_row_, size_t & width_, size_t & height_)
{
  ...
  // load 这个点云耗时将近1s
  auto load_start =  clock();
  pcl::io::loadPCDFile(package_path + "/bag/walk_step/cuting_discard.pcd", cloud);
  std::cout<<"load pcl cost "<<(double)(clock() - load_start) / CLOCKS_PER_SEC <<" s. "<<std::endl;
  width_ = cloud.width; height_ = cloud.height;
  orginazed_points_pcl raw_points_pcl(cloud);
  // raw_points_pcl.initialByPCL(cloud);
  std::cout<<raw_points_pcl.width<<" "<<raw_points_pcl.height<<std::endl;
  std::cout<<raw_points_pcl.pointcloud.width<<" "<<raw_points_pcl.pointcloud.height<<std::endl;
