...
int main(int argc, char** argv)
{
  ...
  // while (!viewer.wasStopped())
	...
	// }
  // system("read -p 'Press Enter to continue...' var");
  pcl::PointCloud<pcl::PointXYZ> left_cy, right_cy;
  for (auto & point : cloud_in_world)
  {
    if (point.z > 0.08 && point.z < 0.8)
    {
      if (point.y > 0)
      {
        left_cy.emplace_back(point);
      }
      else
      {
  ...
  // while (!viewer.wasStopped())
	...
	// }
  // system("read -p 'Press Enter to continue...' var");
  vector<Eigen::Vector2f> left_cy_2d, right_cy_2d;
  for (auto & point : left_cy)
  {
    left_cy_2d.emplace_back(Eigen::Vector2f(point.x, point.y));
  }
  ...
  vector<Eigen::Vector2f> polygen_left, polygen_right;
  // 生成圆上的点
  for (size_t i = 0; i <= 360; i += 20)
  {
    float x = (radius_left + 0.18) * cos(_deg2rad(i));
    float y = (radius_left + 0.18) * sin(_deg2rad(i));
    polygen_left.emplace_back(Eigen::Vector2f(center_left.x() + x, center_left.y() + y));
  }
  ...
  pcl::io::savePCDFile("/home/humanoid/work/plane_detection/catkin_plane_detection_test_ws/src/plane_detection/bag/walk_step/cuting_discard.pcd", cloud);
  viewer.showCloud(cloud.makeShared());
  while (!viewer.wasStopped())
	{
	}
