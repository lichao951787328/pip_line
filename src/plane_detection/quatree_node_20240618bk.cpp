...
node::node(size_t start_rows_, size_t start_cols_, size_t width_, size_t start_rows_2d_, size_t start_cols_2d_, size_t width_2d_, size_t depth_, std::shared_ptr<node> parent_)
{
  ...
  else
  {
    ...
    for (size_t i = 0; i < 4; i++)
    {
      ...
      else
      {
        ...
        children.at(i) = std::make_shared<node>(child_start_rows, child_start_cols, child_width, child_start_rows_2d, child_start_cols_2d, child_width_2d, /* child_binary_code, */ depth + 1, this_ptr);
        // 此情况下也应设为nullptr
