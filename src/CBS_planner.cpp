...
int main(int argc, char **argv)
{
    ...
    obstacle_point.header.frame_id = "map";
    obstacle_point.header.stamp = ros::Time::now();
    obstacle_point.ns = "obstacle";
    obstacle_point.id = 0;
    obstacle_point.type = visualization_msgs::Marker::POINTS;
    obstacle_point.action = visualization_msgs::Marker::ADD;
    obstacle_point.scale.x = 0.05;
    obstacle_point.scale.y = 0.05;
    obstacle_point.scale.z = 0.05;
    obstacle_point.color.r = 1.0;
    obstacle_point.color.g = 0.0;
    obstacle_point.color.b = 0.0;
    obstacle_point.color.a = 1.0;


    vector<Eigen::Vector3d> steps;
    Eigen::AngleAxisd ad(0/180*M_PI, Eigen::Vector3d::UnitZ());
    Eigen::Vector3d start(1.15, -0.4, 0.0);
    ...
    steps.emplace_back(Eigen::Vector3d(start.x(), start.y(), 0));
    steps.emplace_back(Eigen::Vector3d(right.x(), right.y(), 0));
    
    Eigen::Vector3d step3 = Eigen::Vector3d(0.28, 0, 0) + start;
    steps.emplace_back(Eigen::Vector3d(step3.x(), step3.y(), 0));

    Eigen::Vector3d step2 = Eigen::Vector3d(0.56, 0, 0) + right;
    steps.emplace_back(Eigen::Vector3d(step2.x(), step2.y(), 0));

    geometry_msgs::Point p;
    p.x = 1.59;
    p.y = -0.4;
    p.z = 0.04;
    obstacle_point.points.push_back(p);
