...
void localPlanner::setFootParam(FootParam & foot_param_)
{
    foot_param = foot_param_;
}

void localPlanner::setHipWidth(double hip_width_)
{
...
void localPlanner::initial(Eigen::Vector3d start_left_, Eigen::Vector3d start_right_, int support_flag_,Eigen::Vector3d goal_)
{
...
void localPlanner::Inpaint(int radius)
{
...
void localPlanner::detectionPlane()
{
...
void localPlanner::mapPrepare(grid_map::GridMap & map_)
{
    map = map_;
    Inpaint(5);
    detectionPlane();
...
// 必须在mapPrepare 之后。
bool localPlanner::isGoalFeasible(Eigen::Vector3d goal)
{
...
pcl::PointCloud<pcl::PointXYZ> localPlanner::gridMap2PointcloudOrganized(grid_map::GridMap & map)
{
